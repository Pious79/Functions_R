...
##  Pierre L'HERMITE - 2017-07-20 - fc.indice_saisonnalite                       ##
##______________________________________________________________________________##
##------------------------------------------------------------------------------##
#   Fonction : Calcul le jour  donnees avec le filtre passe bas d'Hanning 
#              d'ordre 2
##------------------------------------------------------------------------------##
...
#                                       (defaut 90%)
##------------------------------------------------------------------------------##
#   Sortie    : zoolist [list] : liste contenant 3 zoo indiquant le premier jour
...
fc.indice_saisonnalite<-function(data, debYear="01-01", Pourc_debut=0.1, Pourc_milieu=0.5, Pourc_fin=0.9)
{
  ...
  for (k in (ystart:yend)){
    ...
    Debut<-Pourc_debut*min(cumul, na.rm=T)
    Mid<-Pourc_milieu*min(cumul, na.rm=T)
    End<-Pourc_fin*min(cumul, na.rm=T)
