wavelet.plot2 <- function (wave.list, wavelet.levels = quantile(wave.list$Power, 
                                               probs = (0:10)/10), add.coi = TRUE,
                           add.sig = TRUE, x.lab = gettext("Time"), 
          period.lab = gettext("Period"), crn.lab = gettext("RWI"),main= "", 
          key.cols = rev(rainbow(length(wavelet.levels) - 1)),
          key.lab = parse(text = paste0("\"",gettext("Power"), "\"^2")),
          add.spline = FALSE, f = 0.5, 
          ...
          useRaster = FALSE, res = 150, reverse.y = FALSE, ...) 
{
  ...
  box()
  par(mar = mar3, las = 0)
  plot(x, y, type = "l", xlim, xaxs = xaxs, yaxs = yaxs, asp = asp, main= main, 
